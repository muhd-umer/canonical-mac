/**
 * @file solve_tone.cpp
 * @brief Per-tone L-BFGS optimization implementation
 */
...
bool eval_objective(const VectorXd& x, const std::vector<MatrixXcd>& H_blocks,
                    const VectorXd& delta, const VectorXd& sw,
    ...
    double F = delta.dot(ws.logdet_S) - energy_term;
    phi = -F;
...
ToneResult solve_tone(const MatrixXcd& Hn, const VectorXd& theta,
    ...
    double phi;
    if (!eval_objective(ws.x, ws.H_blocks, delta, sw, sLxu, Ly, ws, phi,
                        ws.grad)) {
        for (int u = 0; u < U; ++u) {
            ws.B_blocks[u] =
                std::sqrt(1e-6) * MatrixXcd::Identity(sLxu[u], sLxu[u]);
        ...
        state_in.S_hist.clear();
        state_in.Y_hist.clear();
        eval_objective(ws.x, ws.H_blocks, delta, sw, sLxu, Ly, ws, phi,
                       ws.grad);
    }
    ...
    for (int iter = 0; iter < cfg.max_iter; ++iter) {
        ...
        for (int ls = 0; ls < cfg.max_linesearch; ++ls) {
            ...
            double phi_trial;
            if (eval_objective(ws.x_trial, ws.H_blocks, delta, sw, sLxu, Ly, ws,
        ...
        if (!accepted) {
            ...
            for (int ls = 0; ls < cfg.max_linesearch; ++ls) {
                ...
                double phi_trial;
                if (eval_objective(ws.x_trial, ws.H_blocks, delta, sw, sLxu, Ly,
    ...
    for (int u = 0; u < U; ++u) {
        ...
        b_sorted(u) = logdet_curr - logdet_prev;
