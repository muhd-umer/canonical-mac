...
TestResult validate_result(const MaxRESMACResult& result, double Etotal,
    ...
    double total_energy = result.Eun.sum();
    double weighted_rate = theta.dot(bu);
    test.weighted_rate = weighted_rate;

    double energy_error = std::abs(total_energy - Etotal) / std::max(Etotal, 1e-12);
    test.energy_error = energy_error;

    std::cout << "  config: " << U << " users, Etotal = " << std::fixed
              << std::setprecision(2) << Etotal << std::endl;

    std::cout << "  weights theta: [";
    for (int u = 0; u < U; ++u)
        std::cout << theta(u) << (u < U - 1 ? " " : "");
    std::cout << "]" << std::endl;
    ...
    std::cout << "  total energy used: " << std::fixed << std::setprecision(6)
              << total_energy << " (target: " << Etotal << ")" << std::endl;
    ...
    if (energy_error < 0.01) {
        std::cout << "  [p] energy constraint satisfied" << std::endl;
    } else {
        std::cout << "  [x] energy constraint violated (error: "
                  << std::scientific << energy_error << ")" << std::endl;
        test.passed = false;
    }
    ...
    for (int u = 0; u < U && rates_ok; ++u) {
        for (int n = 0; n < N && rates_ok; ++n) {
            if (result.bun(u, n) < -1e-6) rates_ok = false;
        }
    }
    if (rates_ok) {
...
TestResult test_basic_siso() {
    ...
    auto result = maxRESMACMIMO(H, Lxu, Etotal, theta, cb);
    return validate_result(result, Etotal, theta, Lxu, "basic siso");
}
...
TestResult test_basic_mimo() {
    ...
    auto result = maxRESMACMIMO(H, Lxu, Etotal, theta, cb);
    return validate_result(result, Etotal, theta, Lxu, "basic mimo");
}
...
TestResult test_single_tone() {
    ...
    auto result = maxRESMACMIMO(H, Lxu, Etotal, theta, cb);
    return validate_result(result, Etotal, theta, Lxu, "single tone");
}
...
TestResult test_unequal_weights() {
    ...
    auto result = maxRESMACMIMO(H, Lxu, Etotal, theta, cb);
    return validate_result(result, Etotal, theta, Lxu, "unequal weights");
}
...
TestResult test_three_users() {
    ...
    auto result = maxRESMACMIMO(H, Lxu, Etotal, theta, cb);
    return validate_result(result, Etotal, theta, Lxu, "three users");
}
...
TestResult test_mimo_mixed_antennas() {
    ...
    auto result = maxRESMACMIMO(H, Lxu, Etotal, theta, cb);
    return validate_result(result, Etotal, theta, Lxu, "mimo unequal antennas");
}
...
TestResult test_real_baseband() {
    ...
    std::vector<MatrixXcd> H(N);
    H[0].resize(1, 2); H[0] << Complex(5, 0), Complex(4, 0);
    H[1].resize(1, 2); H[1] << Complex(3, 0), Complex(2, 0);
    H[2].resize(1, 2); H[2] << Complex(2, 0), Complex(3, 0);
    H[3].resize(1, 2); H[3] << Complex(4, 0), Complex(5, 0);

    auto result = maxRESMACMIMO(H, Lxu, Etotal, theta, cb);
    return validate_result(result, Etotal, theta, Lxu, "real baseband");
}
...
TestResult test_stress() {
    ...
    auto result = maxRESMACMIMO(H, Lxu, Etotal, theta, cb);
    return validate_result(result, Etotal, theta, Lxu, "stress test");
}
...
int main() {
    ...
    std::cout << std::endl;
    results.push_back(run_test("mimo mixed antennas", test_mimo_mixed_antennas));
    std::cout << std::endl;
