...
while 1
    ...
    if min(g) >= 0 % achievable by current vertex
        ...
        Rxxs = Rxxstemp;
        [~, Itheta] = sort(theta);
        info = table(bu_v', bun, Itheta); % detailed info of the vertex
        info.Properties.VariableNames = {'bu_v' 'bun' 'order'};
        return
    end
    ...
    initialinfo = table(bu_v', {bun}, U:-1:1); % detailed info of boundary vertices
    initialinfo.Properties.VariableNames = {'bu_v' 'bun' 'order'};
